...
def tau(i1,i2,gamma,n):
    if i1>=i2:
        ...
        return ((i2-i1) * gamma * np.log(n)).item()
...
def burst_detection(r,d,n,s,gamma,smooth_win):
    ...
    #     temp_p = np.rolling(window=smooth_win, center=True).mean(temp_p)
        windows = sliding_window_view(temp_p, window_shape=smooth_win)
        temp_p = windows.mean(axis=1)

        # make the array the correct length
        for i in range(smooth_win // 2):
            temp_p = np.append(temp_p, np.nan)
            temp_p = np.insert(temp_p, 0, np.nan)
